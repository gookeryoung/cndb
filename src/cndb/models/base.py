...
def __getattr__(name: str) -> Any:  # pragma: no cover - 惰性加载辅助
    ...
    使用者直接 `from cndb.models import User` 时，
    ...
    try:
        module = importlib.import_module(f"cndb.models.{name.lower()}")
        return getattr(module, name)
    except (ImportError, AttributeError) as exc:
        raise AttributeError(f"module 'cndb.models' has no attribute {name!r}") from exc

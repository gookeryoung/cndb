...
@pytest.fixture
def workspace_with_auth(db, client):
    session = db
...
class TestImportCsvAnalyzeApi:
    ...
    def test_analyze_empty_csv(self, client, workspace_with_auth):
        ws_id, auth = workspace_with_auth
        r = client.post(
            f"/api/v1/workspaces/{ws_id}/import-csv/analyze",
            json={"csv_text": ""},
    ...
    def test_analyze_unauthorized(self, client, workspace_with_auth):
...
class TestImportCsvCreateTableApi:
    def test_create_ok(self, client, workspace_with_auth):
        ws_id, auth = workspace_with_auth
        csv_text = "name,age\nAlice,30\nBob,25\n"
        r = client.post(
    ...
    def test_create_empty_csv(self, client, workspace_with_auth):
        ws_id, auth = workspace_with_auth
        r = client.post(
            f"/api/v1/workspaces/{ws_id}/import-csv",
